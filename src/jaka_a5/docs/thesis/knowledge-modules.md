...
图像雅可比矩阵 `J_img` 建立了图像特征速度 `ṡ` 与相机速度 `v_cam` 之间的线性映射：

```
ṡ = J_img × v_cam
```

对于 2D 点特征 `(u, v)`，完整的 6D 相机速度 `v_cam = [v_x, v_y, v_z, ω_x, ω_y, ω_z]`：

```
J_point = [ -1/Z   0   u/Z   u*v  -(1+u²)  v ]
          [  0  -1/Z  v/Z  (1+v²)  -u*v  -u  ]
```
...
```
J_translation = [ -1/Z   0   u/Z ]
                 [  0  -1/Z  v/Z ]
```

### 9.2 Damped Least Squares (DLS)

当雅可比矩阵接近奇异时，伪逆会变得非常大。DLS 通过添加阻尼因子来缓解：

```
J_DLS = J × (J^T × J + λ² × I)^(-1)
```

其中 `λ` 是阻尼因子，通常取 0.01~0.1。
...
```
v_trans = -λ_IBVS × J_img⁺ × e_image
ω_rot = -λ_PBVS × error_rotation
v_cam = [v_trans; ω_rot]
```
